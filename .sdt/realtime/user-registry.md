...
# SDT: ETS-Backed User Registry for Invite Autocomplete
...
In the face of **needing username autocomplete for the invite UI without a database**, instead of doing nothing (**creator must type exact usernames from memory**), we decided **to maintain an ETS table of all seen usernames via a GenServer, populated on every /join, and exposed as a datalist in the invite form**, to achieve **instant autocomplete with zero database overhead**, accepting **that stale usernames accumulate and there's no uniqueness enforcement**.
