...
In the face of **rendering a read-only decision view for non-participants**,
instead of doing nothing
(**no spectator support - only participants can see the decision**),
we decided **to reuse decision_live.ex and conditionally render input controls based on whether the current user is in `decision.connected`**,
to achieve **zero code duplication and a single LiveView that handles both participant and spectator modes**,
accepting **that a bug in the gate condition could accidentally expose inputs to spectators (mitigated by the Core rejecting messages from non-connected users)**.
