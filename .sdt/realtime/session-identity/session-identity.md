...
# SDT: Session-Only Identity (No Authentication)
...
In the face of **needing to identify participants in a multiplayer LiveView app**, instead of doing nothing (**anonymous connections with no way to distinguish users**), we decided **to store a self-chosen username in the Plug session via a simple /join form and POST /session endpoint**, to achieve **instant identity with zero infrastructure**, accepting **that usernames are not unique-enforced, sessions are cookie-bound, and there is no account recovery**.
