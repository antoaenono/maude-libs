...
we decided **to rewrite the layout as a pure, tested Elixir function with no temperature schedule, fixed step size, and convergence detection**,
to achieve **reliable convergence with no new dependencies or architectural changes**,
accepting **the risk of hitting the same tuning issues again, and O(n^2) computation that must complete within a single tick**.
