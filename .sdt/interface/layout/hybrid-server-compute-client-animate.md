...
(**continued oscillation and convergence failures**),
we decided **to call d3-force from the server via a Node.js port, computing positions server-side and broadcasting them via PubSub**,
to achieve **proven d3-force convergence while keeping positions authoritative on the server**,
accepting **a Node.js runtime dependency, port communication overhead, and operational complexity of managing a sidecar process**.
