...
In the face of **structuring UI components for a 7-stage decision flow where 3 stages share layout**,
instead of doing nothing
(**duplicate heex everywhere - inconsistent and hard to update**),
we decided **to use Phoenix function components (def component(assigns) with @doc) for the shared spatial layout skeleton and inline heex for per-stage mechanics**,
to achieve **layout reuse without the stateful LiveComponent overhead**,
accepting **that function components require passing all assigns explicitly (verbose but explicit)**.
