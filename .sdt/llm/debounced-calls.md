...
In the face of **multiple participants submitting scenario text within seconds of each other**, instead of doing nothing (**fire an LLM call on every keystroke/submission, wasting tokens and producing stale results**), we decided **to debounce synthesis calls with an 800ms quiet window, where each new submission resets the timer**, to achieve **a single LLM call that sees all current submissions, fired only after participants pause**, accepting **an 800ms delay before Claude starts thinking (imperceptible to users who are still typing)**.
