...
In the face of **notifying invited participants about a new decision**,
instead of doing nothing
(**invited users must manually navigate to /d/:id - requires out-of-band sharing of the URL**),
we decided **to broadcast invites via per-user PubSub topics (`"user:#{username}"`) so only the invited user receives the notification, with creator-side add/remove list UI backed by UserRegistry autocomplete**,
to achieve **targeted in-app invite delivery that requires no external messaging**,
accepting **that offline users will miss the invite and must navigate directly via URL**.
