...
tags: [llm, http, req]
parent: null
children: [architecture/llm/pluggable-behaviour/pluggable-behaviour]
---

# SDF: LLM API HTTP Client
...
In the face of **making JSON HTTP calls to the Anthropic API from an Elixir app**,
instead of doing nothing
(**no LLM integration - all content is static**),
we decided **to use Req as the HTTP client with manually constructed JSON payloads**,
to achieve **a thin, explicit wrapper where the exact request/response shape is visible in the code without SDK magic**,
accepting **that we maintain our own request construction and response parsing (trivial given the simple JSON contract)**.
