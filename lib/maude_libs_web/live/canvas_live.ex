defmodule MaudeLibsWeb.CanvasLive do
  ...
  def mount(_params, session, socket) do
    ...
    if is_nil(username) do
      ...
      socket =
        socket
        |> assign(username: username, circles: circles, invite: nil)
        |> push_circles(circles)
  ...
  def handle_info({:canvas_updated, circles}, socket) do
    {:noreply, socket |> assign(circles: circles) |> push_circles(circles)}
  end
  ...
  defp push_circles(socket, circles) do
    nodes =
      Enum.map(circles, fn {id, c} ->
        %{
          id: id,
          title: c.title,
          tagline: c.tagline,
    ...
    push_event(socket, "circles_updated", %{circles: nodes})
  end
  ...
  def render(assigns) do
    ...
    <div id="canvas" phx-hook="CanvasForce" class="w-screen h-screen bg-base-200 overflow-hidden relative select-none">
      ...
      <%!-- Circle elements are created by the CanvasForce JS hook --%>
